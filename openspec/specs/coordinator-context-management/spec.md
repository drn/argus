...
The system SHALL provide an `argus coord-hook` subcommand, invoked as a Claude Code `Stop` hook, that on every invocation first checks for a non-empty `ARGUS_TASK_ID` and a resolved hera role kind (`coordinator`, `worker`, or `freelance`); it SHALL no-op with no side effects and no error when `ARGUS_TASK_ID` is empty OR the task carries no hera role binding at all (role kind resolves to `""`). When a role IS bound, the subcommand SHALL: (1) compute a context-size value from the transcript at the hook's supplied `transcript_path` as described below; (2) self-discover the daemon's live REST port and `~/.argus/api-token` (no port/token is exported into the spawned session's environment); (3) overwrite `task_meta` under namespace `hera`, key `context_size`, with the computed value — regardless of role kind. When the bound role is NOT `coordinator`-kind (i.e. `worker` or `freelance`), the subcommand SHALL stop here: it SHALL NOT read or compare against the budget, and SHALL NOT emit a Stop-hook decision. When the bound role IS `coordinator`-kind, the subcommand SHALL additionally compare the value against the project's configured `coordinator_context_budget` — when the value is at or above the budget AND the increment gate below allows it AND (after the hard-stop escalation check below has not fired) `task_meta` (`hera`, `pending_recycle`) is not already `"true"`, the subcommand SHALL emit a Claude Code hook "block" decision whose reason instructs the coordinator to reach a safe seam and call for a reboot, and SHALL overwrite `task_meta` (`hera`, `last_nudged_context_size`) with the current `context_size` value. When `pending_recycle` is already `"true"`, the subcommand SHALL emit no decision, allowing the Stop event to proceed so the session can reach genuine idleness. A failure reading `pending_recycle` SHALL NOT suppress the block decision — it SHALL fall back to emitting it, treating the flag as not-yet-pending.

Computing the context-size value: Claude Code exposes no hook field (on `Stop` or any other event) carrying token usage directly — `last_assistant_message` is documented as the final assistant response TEXT only, never a usage object, and Claude Code's own hooks reference states plainly that hooks needing token usage information must parse `transcript_path` themselves — so the subcommand SHALL scan `transcript_path` for the latest main-chain assistant message's usage and SHALL sum `input_tokens + cache_creation_input_tokens + cache_read_input_tokens` rather than reading `cache_read_input_tokens` alone (summing alone is required because a prompt-cache miss — e.g. an idle gap crossing the cache TTL — rewrites the entire prior context as `cache_creation_input_tokens` instead of `cache_read_input_tokens`, which would otherwise collapse the computed value toward zero even though the real context is unchanged or larger). A transcript line carrying `isSidechain: true` (a dispatched sub-agent's own turn) SHALL be excluded from consideration regardless of its position in the file. Because `transcript_path` is documented by Claude Code as written asynchronously and may not yet reflect the turn that just finished at the exact moment the hook fires, the subcommand SHALL retry the scan across a bounded retry budget, taking the MAXIMUM value observed across all scans in that budget — not simply the first or the last — safe because context size is monotonically non-decreasing within one transcript file — and SHALL use whatever value it has once the budget elapses, without erroring and without waiting indefinitely. To avoid paying this retry budget on every Stop event across an entire hera-managed fleet, the subcommand SHALL first compare a single scan against the task's previously-stamped `context_size` (`task_meta` `hera`/`context_size`, the value this same hook stamped on some earlier Stop event): when that single scan is STRICTLY GREATER than the previous stamp, it SHALL be used immediately with no retry. The subcommand SHALL fall into the bounded retry when the single scan is AT OR BELOW the previous stamp, or when no previous stamp exists yet (the task's first Stop event, with no baseline to judge freshness against); a failure reading the previous stamp SHALL be treated the same as "no previous stamp" — falling open into the retry path rather than trusting an unverified single scan. An EXACT match against the previous stamp is deliberately NOT treated as fresh and MUST fall into the retry: if the current turn's write genuinely hasn't landed yet, the latest main-chain line in the file is still the prior turn's, so a fresh scan comes back exactly equal to what was already stamped — the precise undercount race this fix exists to catch.

The increment gate (coordinator-only): the subcommand SHALL read `task_meta` (`hera`, `last_nudged_context_size`) and the project's configured `coordinator_nudge_increment` (default 50000). When `last_nudged_context_size` is unset, OR the current `context_size` is less than `last_nudged_context_size`, the gate SHALL allow the nudge (a first-ever nudge, or a fresh over-budget episode following a drop back below budget — e.g. immediately after a recycle reset accumulated context — both nudge immediately with no increment wait). Otherwise, the gate SHALL allow the nudge only when `context_size >= last_nudged_context_size + coordinator_nudge_increment`, and SHALL suppress it otherwise. A failure reading `last_nudged_context_size` or `coordinator_nudge_increment` SHALL NOT suppress the block decision — it SHALL fall back to allowing the nudge (treating the read as if no prior nudge exists, or the increment as 0), consistent with `pending_recycle`'s existing fail-open precedent.
...
- **WHEN** the Stop hook fires in a session with no `ARGUS_TASK_ID` set, OR with `ARGUS_TASK_ID` set but the task carrying no hera role binding at all (role kind resolves to `""`)
- **THEN** it exits with no side effects — no `task_meta` write, no blocking decision

#### Scenario: A cache miss does not undercount context

- **WHEN** the latest main-chain assistant message's usage shows `cache_read_input_tokens: 0` because a prompt-cache miss rewrote the whole prior context as `cache_creation_input_tokens`
- **THEN** the computed context-size value still reflects the full prior context, since it sums all three usage fields rather than reading `cache_read_input_tokens` alone

#### Scenario: A sidechain sub-agent turn does not override the main chain's usage

- **WHEN** a transcript line with `isSidechain: true` (a dispatched Task-tool sub-agent's own turn, carrying a small usage number unrelated to the main conversation) lands physically after the main chain's latest assistant message
- **THEN** the computed context-size value reflects the main chain's usage, not the sidechain line's

#### Scenario: A scan strictly above the previous stamp skips the retry

- **WHEN** a single scan of `transcript_path` returns a value strictly greater than the task's previously-stamped `context_size`
- **THEN** that value is used immediately, with no retry performed

#### Scenario: A scan at or below the previous stamp falls into the retry

- **WHEN** a single scan of `transcript_path` returns a value AT OR BELOW the task's previously-stamped `context_size`
- **THEN** the subcommand falls into the bounded retry rather than trusting that scan

#### Scenario: An exact tie with the previous stamp is not treated as fresh

- **WHEN** a single scan of `transcript_path` returns a value EXACTLY EQUAL to the task's previously-stamped `context_size` — the latest main-chain line in the file is still the prior turn's because the current turn's write hasn't landed yet
- **THEN** the subcommand falls into the bounded retry rather than short-circuiting on the stale, unchanged value

#### Scenario: A task's first-ever Stop event retries with no baseline to compare against

- **WHEN** a single scan of `transcript_path` is performed for a task with no previously-stamped `context_size` at all
- **THEN** the subcommand falls into the bounded retry rather than trusting that scan unconditionally

#### Scenario: A transcript write still landing within the retry budget is captured

- **WHEN** `transcript_path` does not yet reflect the turn that just finished at the moment the Stop hook starts scanning, but the async writer flushes that write before the retry budget elapses
- **THEN** the computed context-size value reflects the freshly-flushed write, not the stale value present at the first scan

#### Scenario: A transcript write landing after the retry budget is not captured

- **WHEN** `transcript_path`'s write for the turn that just finished has not landed by the time the retry budget elapses
- **THEN** the subcommand uses the largest value it was able to observe within the budget, without erroring and without waiting past the budget

#### Scenario: Context size is stamped for a worker or freelance session, with no budget enforcement

- **WHEN** the Stop hook fires in a session bound to a `worker`-kind or `freelance`-kind role
- **THEN** `task_meta` (`hera`, `context_size`) is overwritten with the latest computed value, and the subcommand emits no Stop-hook decision and never reads the configured budget, regardless of how large the value is
