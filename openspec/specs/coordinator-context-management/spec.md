...
#### Scenario: A scan already at or above the previous stamp skips the retry

- **WHEN** a single scan of `transcript_path` returns a value at or above the task's previously-stamped `context_size`
- **THEN** that value is used immediately, with no retry performed

#### Scenario: A scan below the previous stamp falls into the retry

- **WHEN** a single scan of `transcript_path` returns a value BELOW the task's previously-stamped `context_size`
