...
- **THEN** its "Hera View (rail)" section lists `j`/`k`, space, Left, Tab/Ctrl+Q, Enter, `w`, `n`, `r`, `a` (hide), `P`, `s`/`S`, `m`/`M`, `J`, `B` (force recycle), `C` (clear archive), `Ctrl+Z`, `/`, and `Ctrl+D` (nuke), and does NOT list `R` or a rail-wide `Ctrl+R`
