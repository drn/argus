...
This capability packages argus's own argus-coupled skills (skills that drive `mcp__argus__*` tools, read `ARGUS_TASK_ID`/`~/.argus`, or encode the hera coordination model) directly into the argus binary, so every Claude-backend task argus boots gets them automatically — with no dependency on an external repository (e.g. `~/.dots`) and no risk of the skill text drifting from what the installed binary actually implements. Skills are embedded via `go:embed`, materialized on demand to a managed workspace directory, and made available to Claude Code via the `--add-dir` flag (`agent-execution`'s command-construction requirement), which loads `.claude/skills/` from an added directory as a documented exception to `--add-dir` otherwise granting file access only.
