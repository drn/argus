...
- **THEN** it includes `hera-spawn-review` and `argus-resolve-model` alongside the pre-existing `argus-archive`, `argus-complete`, `argus-schedule`, `hera`, `hera-plan`, `hera-review`, and `hera-review-test-adversary` entries
