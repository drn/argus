...
- **Collapsed group = dashed box** (`┌╌╌┐ / ╎ … ╎ / └╌╌┘`, `layoutDashedBox`) holding the `[a–c]` range label + the `groupCounts` line.
- **Fanned group = dashed enclosure** (`layoutFannedGroup`) wrapping the member node-boxes laid out horizontally, with a top-edge label (`┌╌ research ╌┐`) = the members' common role token (`commonRoleToken`) or the range. The `↘` partial-feed marker rides the feeding member's box content (D5).
