...
- **Selection = a distinct box border, not reverse-text** (`selectionBorderStyle`): bright pink (`ColorSelected`) + bold when the widget owns focus, cyan (`ColorTitle`) when it does not. The cursor's node — or, inside a fanned group, the cursor's member box — gets this border. This is the user's explicit "more of a box" ask; the old reverse-on-text highlight was dropped.
