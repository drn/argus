...
**Render note — centered tight tree + cursor highlight + fanned expansion.** Each stage row is centered horizontally within the diagram region (`inner.X + max(0, (inner.W - rowWidth)/2)`, where `rowWidth` is the rune-aware sum of chip widths plus inter-chip gaps) and the whole block is centered vertically when shorter than the region, matching the web artifact's centered tight tree rather than left/top-aligning at `inner.X`/`inner.Y`. The chip under the cursor is drawn with a reverse-video highlight (bold when the widget owns focus) so the selected node is visible in the diagram, not only in the header. A **fanned-out group expands in the diagram**: `drawStages` builds the row's chips via `stageRowChips`, which emits one `glyph + short-id` member chip per member (with the `↘` partial-feed marker on the feeding member, D5) in place of the collapsed `[a–c]` range box, and highlights the cursor's member; the centering width tracks the wider expanded row. A collapsed group still renders the range box. All pure render concerns — no `Sync`, full-rect coverage preserved.
