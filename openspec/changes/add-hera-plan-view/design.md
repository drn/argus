...
- **Node = padded rounded box** (`╭─╮ / │ glyph short-id │ / ╰─╯`, `layoutNodeBox`), 3 rows, one space of inner padding; the border runes take the node's state colour (`State.style()` foreground).
