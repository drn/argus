...
- **TOCTOU Ctrl+U race (accepted, narrow):** `processOne` passes the `cancelCh` check, then `Cancel()` fires and removes the delivery from `pending` before `processOne` reaches the PTY write. Result: a stray Ctrl+U + text + CR lands in the PTY despite the cancel call. No double-submit occurs (the delivery is no longer in pending so `removeAndAdvance` is a no-op). The window is a few nanoseconds; the practical probability at the 3s idle threshold is negligible. Accepted.
- **Daemon restart drops all pending deliveries (accepted non-goal):** The `Notifier` is in-memory. If the daemon restarts, all pending deliveries are lost silently. Callers (hera, MCP) must re-post after reconnecting. Sessions also restart fresh, so any in-flight PTY state is lost anyway — this is not a net regression. No persistence work planned.
