...
- [ ] 2.1 Add an alias-safe query builder: one `repository(owner,name)` block with `tN:` aliases (N = sanitized task id) of `pullRequests(headRefName:…, first:1, orderBy:{field:CREATED_AT,direction:DESC}){ nodes{ state isDraft reviewDecision url } }`, plus a top-level `rateLimit { cost remaining }` field
- [ ] 2.2 Implement `FetchPRStatesBatch(ctx, repo, branches) (map[branch]PRResult, error)`: write the query to a temp file, invoke `gh api graphql -F query=@file` via an injectable runner seam (default real `gh`), parse JSON, map each alias back to its branch, reuse the existing state-mapping logic from `FetchPRState`/`prstate.go`
- [ ] 2.3 Implement the per-query alias cap (default 100) with sequential chunking inside `FetchPRStatesBatch` (or a thin wrapper), so callers pass a whole repo group and chunking is internal
- [ ] 2.4 Keep `FetchPRState` (single) in place as a fallback / non-poller helper; do not delete it in this change

## 3. Repo resolution + poller wiring

**Depends on:** Stage 1

- [ ] 3.1 Add a repo-resolution helper: parse `owner/name` from a cached `pr`/`url`; fall back to the worktree's default GitHub repo (the repo `gh` targets for that dir), resolved off the UI thread like existing git ops
- [ ] 3.2 Rework `pollPRStatesOnce` to: filter eligible tasks (unchanged terminal-skip), resolve each task's repo, group by `(owner,name)`, call `FetchPRStatesBatch` once per group, then apply per-branch results to `task_meta` with the existing keep-stale (group error) / authoritative-write (success, incl. `none`) semantics
- [ ] 3.3 Extend the uxlog: keep the per-cycle `[pr] poll: eligible/skipped/written/errored` summary; add a per-repo `[pr] poll: repo=o/n branches=K cost=C` line (cost from the query's `rateLimit`)
- [ ] 3.4 Inject the batch runner into the daemon the same way `prFetch: gitutil.FetchPRState` is bound today, so tests can substitute a fake
...
- [ ] 4.2 Add a gotcha to `context/knowledge/gotchas/` (daemon-rpc.md or misc.md): `gh pr view`/`pullRequests` hit the GraphQL bucket not REST; batch by repo; `pullRequests(headRefName:)` survives branch deletion where `ref(...)` returns null; cost = ceil(nodeCount/100)
