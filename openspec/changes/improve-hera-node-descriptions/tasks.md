...
- [ ] 1.1 Update the `prompt` param description on `hera_spawn_worker`
...
- [ ] 1.2 Update the `prompt` param description on `hera_plan_node`
  (`internal/mcp/hera_plan.go`) with the same guidance.
- [ ] 1.3 Confirm no logic change: the supplied `prompt` is still stored verbatim
  on the role (no stripping, no transformation).
- [ ] 1.4 Test: assert both tool schemas' `prompt` descriptions contain the
  mission-only guidance and do not instruct prepending policy.

## 2. Hera skill guidance (`.claude/skills/hera/SKILL.md`)

- [ ] 2.1 Add a rule in the skill's spawn/plan sections: pass the mission only;
...
- [ ] 2.2 Include the one-line rationale (harness auto-injects; verified) so the
  guidance is self-justifying.

## 3. Plan-DAG node description render (spec: hera-view)

- [ ] 3.1 Change the node description source from `firstLine(r.Prompt)` to the
...
- [ ] 3.2 Wrap the description to the detail-pane width; grow the header line
  count accordingly without breaking the roster/graph split in the coordinator
  Details region.
- [ ] 3.3 Keep it policy-agnostic: no stripping, no skipping, no line-1
  assumption. Preserve the `"(no description)"` placeholder for an empty prompt.
- [ ] 3.4 Test: a multi-line prompt renders several header lines (not just the
...
- [ ] 4.1 `make pre-pr` passes clean (build → vet → fmt-check → lint-pr → vuln →
  test-cover-gate).
- [ ] 4.2 Add/update gotchas in `context/knowledge/gotchas/hera-view.md` (node
