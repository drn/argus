...
- [x] 1.5 Write failing tests for the Claude session switcher (toolbar button opens picker, selecting a session attaches to it) — RESOLVED via Stage 6a's new daemon endpoints (approved by coordinator); tests landed as part of Stage 6
...
- [x] 7.4 `openspec archive add-mac-keybinding-parity` (or the manual merge-and-move fallback) on the change branch before merge, per this repo's CLAUDE.md — archived as `2026-08-21-add-mac-keybinding-parity`; deltas merged into `openspec/specs/macos-app/spec.md` and `openspec/specs/rest-api/spec.md`
