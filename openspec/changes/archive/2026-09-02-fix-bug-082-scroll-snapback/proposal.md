...
**BUG-082 — in the Hera split view, scrolling up a live-looking agent pane "scrolls up a line and then jumps back to the bottom, repeatedly"; the pane cannot be scrolled at all until the TUI is restarted.** Reported against the Coordinator/Agent dual-pane terminals; the classic fullscreen single-agent view is unaffected.
...
- **New tests:** `internal/tui/terminal/bug082_test.go` (predicate, snap-back repro, accelerated scroll, main-screen control case, live-emulator precedence, reset clearing, hint wording) and `internal/tui/hera/bug082_test.go` (end-to-end wheel and PgUp over a dead-handle Hera pane).
- **Second exposure, covered by the same fix:** a Hera pane bound to a task with **no session handle at all** also renders through the replay path. This only became reachable on `master` via `fix-closeout-replay-load-order` (#943's follow-up), which moved the eager `loadSessionLog` into `SetSession` — before it, `ResetVT` wiped the `replayData` that `SetTaskID` had just loaded, `HasContent()` stayed false, and `Draw` short-circuited to the "Session not running" placeholder without ever replaying. Both exposures are covered here and both are tested.
