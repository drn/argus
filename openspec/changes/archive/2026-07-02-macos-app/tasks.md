...
- [x] A.1 `macos/Package.swift`: SwiftPM package with `ArgusKit` (library) and
      `ArgusMac` (executable, depends on `ArgusKit` + `SwiftTerm`).
- [x] A.2 `ArgusKit`: `ArgusClient` (Bearer-auth `URLSession` wrapper),
...
- [x] C.1 `ArgusKit`: stop / restart / resume / archive / unarchive / rename /
      delete / fork calls against their `/api/tasks/{id}/*` endpoints.
- [x] C.2 `ArgusKit`: `GET /api/tasks/{id}/git/diff`, `GET
      /api/tasks/{id}/git/status`, `GET /api/tasks/{id}/files`, `GET
      /api/tasks/{id}/links`.
- [x] C.3 `ArgusKit` tests for each new call (request shape, response decode,
      error surfaces on 404/missing task).
- [x] C.4 `ArgusMac`: task action menu (rail context menu + detail toolbar)
      wired to C.1, with a native confirmation dialog gating stop/delete.
- [x] C.5 `ArgusMac`: Diff tab (unified-diff rendering with add/remove
...
- [x] D.1 `ArgusKit`: `GET /api/events/stream` named-event SSE client with
      since-cursor tracking and resync-on-invalid-cursor handling.
- [x] D.2 `ArgusMac`: wire the events stream to rail updates (no polling);
      full `GET /api/tasks` resync path on cursor-invalid.
- [x] D.3 `ArgusMac`: `UNUserNotificationCenter` integration — native
...
- [x] D.4 `ArgusKit`: `GET /api/hera` roster fetch + role-tree model.
- [x] D.5 `ArgusMac`: Hera tab rendering the roster read-only (no mutation
      controls), matching web-app scope.
- [x] D.6 `ArgusKit`: `GET /api/schedules`, `POST /api/schedules/{id}/run`.
- [x] D.7 `ArgusMac`: Schedules list view with a "Run now" action.
- [x] D.8 `ArgusMac`: Settings surface (server URL + token override,
...
- [x] D.9 Tests: events resync behavior, notification-suppressed-when-focused
...
- [x] E.1 `make mac-app`: script/target assembling `ArgusMac.app` (Info.plist,
...
- [ ] E.2 README: new Reference section documenting the macOS app (build/run,
      ...
      (deferred — follows as a separate docs PR after feature stabilizes)
- [x] E.3 CLAUDE.md: add the three-surface frontend parity rule (TUI / web
      app / macOS app; parity scoped to the REST-exposed surface; gaps
      recorded as named follow-ups).
- [x] E.4 `context/knowledge/gotchas/`: new file (or `misc.md` section) for
...
- [x] E.5 `swift test --package-path macos` green across `ArgusKit`; manual
      smoke pass of `ArgusMac` against a local daemon for each phase's
      scenarios.
- [x] E.6 `make pre-pr` green (Go tree unaffected but must stay green).
- [x] E.7 Archive this change: fold the delta into
