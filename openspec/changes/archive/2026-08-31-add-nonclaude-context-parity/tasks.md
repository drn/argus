...
- [x] 6.3 Manual smoke test: spawn a real Codex session and confirm (a) it discovers argus's builtin skills via `codex debug prompt-input` or a live session and (b) its prompt carries the CLAUDE.md + routing prefix. **Done, through the real code path** (a throwaway `go run` harness calling the actual, non-test `agent.BuildCmd` — not a script reimplementing the prefix logic — against a real worktree with a marker `CLAUDE.md`), then fed the exact generated command into `codex debug prompt-input`. Confirmed in one rendered prompt: both `SMOKETEST-REPO-CLAUDEMD-MARKER` and `SMOKETEST-MARKER-PROMPT` present (CLAUDE.md + routing prefix + original prompt all correctly threaded through), and all 9 builtin skills listed in Codex's own `<skills_instructions>` catalog at their real `$CODEX_HOME/skills/<name>/SKILL.md` paths (confirming `EnsureCodexSkills()` materialized them for real and Codex's native discovery picked them up). No wiring bugs found. Harness deleted after use; the materialized `~/.codex/skills/` output was left in place (the correct, intended production artifact, not a test fixture).
- [ ] 6.4 Manual smoke test: spawn a real opencode session and confirm (a) it discovers argus's builtin skills via the injected `skills` config entry, (b) its prompt carries routing orientation only, and (c) it still independently picks up CLAUDE.md content on its own (no duplication). **Not performed** — no opencode CLI installed in this environment; rests on opencode's official docs plus unit-level test coverage of the injection logic. **Named follow-up, not silently dropped**: called out explicitly in the PR description as required before relying on the opencode path in production.
