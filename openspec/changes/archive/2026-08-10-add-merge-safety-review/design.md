...
- **Does the classifier need a fallback signal for a Hera-descended worker task that was folded into its parent coordinator's branch via a plain `git merge` and never got its own standalone PR?** Found live (2026-08-10), a real gap: Tier B (PR-name lookup) correctly finds nothing for a folded-in worker, since no PR ever existed for it — not a Tier B bug. A commit-SHA-based fix (capture the task's final commit SHA before its branch is deleted, then check ancestry against master) was proposed to close this, but does not work either: `drn/argus` squash-merges every PR to master, and a squash produces a brand-new commit whose parents don't include any of the original branch's commits — so once a PR lands, neither the branch name nor its SHA is ever an ancestor of master again. The only remaining signal would be inference from the coordinator's own confirmed-merged status, which is a real weakening of this classifier's stated "never guess toward safe" invariant (openspec `add-merge-safety-classifier`) and needs an explicit decision from Aaron — whether that inference is acceptable, and if so under what conditions — not a silent default either way. Not attempted in this change.
