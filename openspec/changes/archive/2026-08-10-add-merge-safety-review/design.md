...
None outstanding. The classifier-gap question below was resolved by Aaron the same night it was
raised and implemented in `add-coordinator-inferred-safety` (stacked directly on this change):

- **RESOLVED — Does the classifier need a fallback signal for a Hera-descended worker task that was folded into its parent coordinator's branch via a plain `git merge` and never got its own standalone PR?** Found live (2026-08-10): Tier B (PR-name lookup) correctly finds nothing for a folded-in worker, since no PR ever existed for it, and a commit-SHA-ancestry fallback doesn't work either — `drn/argus` squash-merges every PR, so neither the branch name nor its SHA is ever an ancestor of master once a PR lands. Aaron's decision: accept a bounded, one-hop inference from the coordinator's own confirmed-safe classification (structurally incapable of recursing past one hop — the coordinator lookup has no orchestrator awareness) rather than leaving the gap unresolved. Shipped as `add-coordinator-inferred-safety`: `mergesafety.TierCoordinatorInferred`, `db.CoordinatorTaskForOrchestrator`, and a distinct "(safe via coordinator)" popup annotation so the inference is never silent.
