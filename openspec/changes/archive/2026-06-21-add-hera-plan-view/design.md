...
A header strip renders above the diagram inside the same bordered panel (the artifact's `.detail` region): for a node, `Name` / `Status` / `Description` / `Feeds`; for a collapsed group, `Group` (range · title) / `Members` / `Downstream`. The `Status` line (BUG-006, an addition beyond the artifact the user asked for) shows the node's state word + glyph via `State.Label()`/`State.Glyph()` — `planned` for a never-bound role, else the live worker's state. "Description" is the role's delivery prompt's first line (the substrate persists the prompt on the planned role); "Feeds"/"Downstream" derive from the edge set. The header height is fixed and budgeted exactly — `headerContentRows` is 4 (bumped 3→4 for the Status line) plus a separator rule (mirroring `DetailsView.ContentHeight`'s discipline) so the diagram gets the remainder without truncation drift regardless of the cursor target (the group view returns 3 lines, padded to 4).

**Render note — the boxed tight-tree treatment (the approved artifact).** `drawStages` lays each stage out as a `stageBlock` (a width/height + a clipped draw closure) and stacks them with a centered `│` connector:

- **Node = padded rounded box** (`╭─╮ / │ glyph short-id │ / ╰─╯`, `layoutNodeBox`), 3 rows, one space of inner padding; the border runes take the node's state colour.
- **Node glyph = 1:1 with the rail (BUG-007).** A LIVE node's glyph+style come from a stamped `Node.Icon`, computed by the projection (`planNodeIcon` in `internal/tui/hera/plan.go`) through the SHARED classifier `widget.RoleStatusIcon(roleStatusInputs(role), …)` — the exact same function the rail's `statusIcon` now calls, so the two can't drift. The widget renders `Node.Icon` via `nodeGlyph`, re-resolving `widget.SpinnerFrame(animFrame)` at Draw for the `Animated` (genuinely-active) case so it animates in lockstep with the rail (`animFrame` recomputed at the top of each Draw by `planSpinnerFrame`). The two plan-view-specific overlays the rail lacks — planned `○` and failed `✕` — leave `Node.Icon` nil and fall back to `State.Glyph()`/`State.style()`. The header Status line (`headerStatusGlyph`) uses the same resolved icon. The shared classifier lives in `widget` (not `theme`) because both `hera` and `planview` import it and it needs the spinner; planview never imports hera (cycle).
- **Selection = a subtle background FILL across the whole box** (primary cue, BUG-004) plus a bright border foreground (secondary). The cursor's box (node, collapsed group, or the cursor's member box inside a fanned group) fills every cell — border + interior padding + content — with `selectionBG` (= the theme's existing `ColorHighlight`, a dark gray only slightly lighter than the default terminal background, reused for consistency with the rest of the TUI's cursor highlighting) via `fillRect` painted under the box, then the border (`selectionBorderStyle`: pink `ColorSelected`+bold focused / cyan `ColorTitle` unfocused) and content paint on top carrying the same background. Foreground stays readable because only the background changes. A border-colour-only swap (orange→pink) was too imperceptible to track — the fill is what makes the cursor unmistakable; the old reverse-on-text highlight stays dropped.
- **Collapsed group = dashed box, TWO lines** (`┌╌╌┐ / ╎ … ╎ / └╌╌┘`, `layoutDashedBox`) matching the design (BUG-005): top line `[a–c]` + a **feed indicator** (`groupTopLine`) — `→ <short-id>` when every out-of-group edge points to ONE node (full feed), `↘` when only some members feed (partial), nothing otherwise; sub line `<role token> · <per-state counts>` (`groupSubLine` = `commonRoleToken` + `groupCounts`). The bare planned-count that used to trail the range (read as "blocks N") is gone. Feed semantics live on `Group` (`FeedTarget` / `PartialFeed` / `FeedingMembers`), computed in `buildGroup`: partial (some members don't feed) takes priority over the single-target arrow.
- **Fanned group = SOLID rounded enclosure** (`layoutFannedGroup`) wrapping the member node-boxes laid out horizontally, with the common role token rendered VERTICALLY down the left inner edge (one rune per row, clipped to box height — a faithful-enough take on the design's vertical label; terminals can't rotate text) and a `▲` collapse affordance at the top-right. Each member in `FeedingMembers` carries a `↘` on its box (the design's `2d ↘ 2e ↘ 2f ↘`). The cursor's member box takes the selection fill; the enclosure itself fills only when the cursor rests on the group slot with no member selected.
- **Centering**: each stage block is centered horizontally (`inner.X + (inner.W - block.width)/2`, rune-aware over the box widths) and the whole block vertically when it fits.
- **Footer hint bar**: a dim bottom inner row (`↑↓ stage · ←→ within · Enter fan · Esc back`); the diagram region loses that row.
- **Vertical scroll** (boxes are 3× taller, so tall plans overflow): `scrollOffsetFor` computes a region-relative offset from the cursor's stage so its box stays fully visible (anchors the top, nudges so the bottom fits, clamped to `[0, totalH-regionH]`); offset 0 when the block fits. Every cell is painted through a `clipRect` so an overflowing/scrolled box never writes outside the region or over the footer.

All pure render concerns — no `Sync`, full-rect coverage preserved by `DrawBorderedPanel` + the clip.
