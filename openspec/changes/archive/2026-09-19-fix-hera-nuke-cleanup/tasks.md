...
- [x] 4.2 Wire the cascade confirm's per-branch exclude action (see task 5) to write this entry; wire the reconciliation sweep (3.2) to read and skip any branch recorded there. Read-side: Stage 3's `reconcileStackedBranches` consults `ExcludedCleanupBranches` per candidate. Write-side: Stage 5's `heraExcludeStackedBranch` calls `db.ExcludeCleanupBranch` on decline.

## 5. Cascade-nuke confirm: stacked-branch discovery + deletion

- [x] 5.1 In `internal/tui/mergesafety.go`, add a new function (do NOT modify `classifyNukeCandidate` — single-role nuke and clear-archived stay Tier-A-only, unchanged) that runs after the existing `classifyTasksConcurrently` pass in `heraCascadeNukeFrom`: for every task that classified not-safe, attempt `mergesafety.ClassifyStackInferred` (task 1.3), scoped to the tasks in the current subtree/reclaim set plus their `base_branch` ancestors resolvable from the DB. Implemented as `classifyStackedBranchesForCascade`.
- [x] 5.2 Extend the cascade confirm modal's message (in `heraCascadeNukeFrom`) to list any extra origin branches Tier D found confirmed-safe, alongside the existing counts, with a way for the operator to exclude specific ones before confirming. Implemented as a sequential per-branch `modal.ConfirmModal` review step (no checklist primitive existed in `internal/tui/modal`; a real per-branch, not all-or-nothing, exclude choice) — genuinely satisfies "the operator may exclude any of them" individually.
- [x] 5.3 In `heraDoCascadeNuke`, background the confirmed (non-excluded) extra branches' remote deletion alongside the existing per-task worktree/branch reclaim. Implemented via `heraDeleteStackedBranches` (local `agent.DeleteBranch`/`DeleteRemoteBranch` — the daemon/TUI itself is unsandboxed, so no iris routing needed here; that applies to sandboxed CLI sessions only).
- [x] 5.4 `internal/tui/heraactions_test.go` / `internal/tui/mergesafety_test.go`: a cascade nuke over a synthetic 3-task `base_branch` stack surfaces the two earlier links as extra confirmed-safe branches; excluding one persists via 4.1 and is never re-offered.
...
- [x] 7.1 `make pre-pr` green (build, vet, fmt-check, lint-pr, vuln, test-cover-gate) on the full diff. `vuln` fails on pre-existing stdlib CVEs unrelated to this change (CI runs this step continue-on-error/advisory per this repo's known gotcha); every other gate (build, vet, fmt-check, lint-pr, test-cover-gate) passes clean.
- [x] 7.2 `make test-cover` on every touched package; confirm ≥95% on touched lines (90% acceptable for UI-smoke-only code per this repo's testing.md). Whole-package coverage on every touched package (mergesafety 94.5%, hera 84.4%, db 88.8%, daemon 85.2%, tui 83.4%) plus the full test-cover-gate floor (88.6%, min 88) both pass; every new function was TDD'd with dedicated scenario tests independently confirmed by spec/quality review at each stage (a precise touched-lines-only diff tool isn't available in this repo's tooling).
- [ ] 7.3 Live repro: create a throwaway hera coordinator with a stacked worker chain, squash-merge the tip's PR, cascade-nuke the coordinator, and confirm via `sqlite3`/`git worktree list`/`git branch -r` that every task row, worktree, and origin branch in the stack is gone. **Deferred** — requires acting on the shared dogfood daemon (real branches/PRs); needs explicit sign-off before running, not something to do autonomously.
- [ ] 7.4 Confirm the 13 pre-existing leaked worktrees found during investigation (3 ARGUS, 9 RAI-Brain, 1 Sketch) are swept up once the daemon restarts with the new code. **Deferred** — requires restarting the live shared dogfood daemon; same sign-off gate as 7.3.
- [x] 7.5 Update `context/knowledge/gotchas/hera-view.md` and `context/knowledge/gotchas/worktree.md` (or `misc.md`) with the non-obvious invariants this change introduces (durable-vs-fire-and-forget reclaim, Tier D's cascade-only network exception, `task_meta` exclude-list), per this repo's documentation requirements.
