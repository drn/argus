...
## Purpose
Let users remap argus's own TUI keybindings via context-scoped `~/.argus/config.toml` tables, resolved by a single `keymap` package that drives every dispatch site and the help overlay, with live reload — within documented limits that protect the agent PTY and structural navigation.
