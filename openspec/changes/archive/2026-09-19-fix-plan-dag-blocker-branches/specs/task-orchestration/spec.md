...
#### Scenario: Fan-in worker receives every blocker branch

- **WHEN** a planned node with two or more blockers materializes
- **THEN** its initial prompt names every blocker and resolved branch, identifies the blocker selected as `base_branch`, and marks any unresolved blocker branch as unavailable
...
- **THEN** no fan-in branch section or fan-in notice is produced (there is no sibling branch to report)
