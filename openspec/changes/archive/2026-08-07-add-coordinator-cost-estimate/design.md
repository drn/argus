...
- **REST** (`internal/api/hera.go`): STILL extended — `heraRoleJSON` (lines 15-27) gains per-role token totals and `cost_usd`, and `heraOrchJSON` (lines 32-43) gains a `subtree_cost_usd` field (the Decision-4 rollup). This is not optional scope-creep: the native TUI itself reads through this same REST surface in `--remote` mode (per the `remote-tui` capability's `apistore`/`apiclient` architecture), so the fields are load-bearing for TUI-only rendering, not just a courtesy to the deferred clients. Shipping them now also means the named follow-up below needs no further backend work.
  - **Implementation-discovered scope note:** `handleHera`'s own doc comment states it "mirrors `hera.BuildModel`'s read logic but emits JSON directly from the db methods so the API package stays free of TUI deps" — and `internal/tui/hera` is ONE Go package (Model/BuildModel and the tview-based rail share it), so importing any part of it for the REST handler would pull tview/tcell into the API binary, not just the pure logic. Reproducing `Model.BridgeSubtree`'s cross-orchestrator traversal independently in `internal/api`/`internal/db` was judged not worth the duplication risk for a field whose only current consumer (remote-mode TUI) isn't yet shipped. REST's `subtree_cost_usd` is therefore SCOPED to the orchestrator's own roles (including nuked ones) — NOT a recursive walk into nested sub-coordinators. Local-mode TUI is unaffected: `Model.SubtreeCostUSD` computes the full recursive figure directly, with no REST round-trip. A true cross-orchestrator REST rollup is a named follow-up.
...
**Implemented** (hera message #4774 — Aaron's explicit go-ahead following this design). See `tasks.md` for the completed stage-by-stage breakdown and the two scope notes discovered along the way (REST/Details-pane cost figures are non-recursive; see Decision 5).
