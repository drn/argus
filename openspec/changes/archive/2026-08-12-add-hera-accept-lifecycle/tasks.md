...
## 11. Sibling gap: the size-drift kick's auto-restart must also refuse a closed-out worker

**Depends on:** Stage 9 (found via `daemon.log` correlation on a live sibling task in this same workstream — a coordinator-accepted worker's row was merely navigated onto, at a different terminal width, and the size-drift kick silently restarted it with zero `Enter` press involved)

- [x] 11.1 `internal/db/hera.go`: new `TaskHoldsLiveHeraWorkerOrFreelanceBinding(taskID)` — the `hera.Selection.IsWorkerOrFreelance()` scoping test for callers with only a bare task ID, no `Selection`. Mirrors `TaskHoldsLiveHeraWorkerBinding` but includes freelance-kind bindings.
- [x] 11.2 `internal/tui/heraactions.go`: new `heraKickRestartClosedOut(taskID)` — resolves the worker/freelance scoping via 11.1, then delegates to the SAME `heraTaskClosedOut`/`HeraWorkerAwaitingCloseout` predicate Stage 9 introduced (reused, not reimplemented).
- [x] 11.3 `internal/tui/app.go`: `handleSessionExitUI` snapshots `kickClosedOut`/`kickClosedOutErr` immediately after the initial `db.Get`, gated on the same `!cleanExit && a.pendingRerenderRestart[taskID]` condition the restart branch itself checks — BEFORE the `StatusInProgress` branch's BUG-050 roll can run. The `pendingRerenderRestart` branch then consults that snapshot before flipping status/calling `startSession`: on an error, fails closed (skips the restart, mirrors the Enter-path guard); when closed out, skips the restart silently (no status write, no session start, task left exactly as close-out left it). Coordinators are unaffected (unconditional restart, unchanged). The pre-transition snapshot is load-bearing: evaluating the guard AFTER the `StatusInProgress` branch runs would misread a healthy, merely-idle in-progress worker's own fresh `ready_to_close` stamp (a side effect of that same exit) as a pre-existing close-out.
- [x] 11.4 `internal/tui/heraactions_test.go`: `TestHandleSessionExitUI_RerenderRefusesClosedOutWorker` (table-driven: accepted-complete + ready_to_close, self-reported-done in_review + ready_to_close, in_review + terminal role-status done) — cursors onto a worker row via the Hera tab, fires the kick's exit path directly, asserts the restart is refused (status unchanged, no `SessionID` set).
- [x] 11.5 `internal/tui/heraactions_test.go`: `TestHandleSessionExitUI_RerenderStillRestartsNonClosedOutWorker` regression-guards the pre-transition-snapshot ordering — a genuinely in-progress (not closed-out) worker still restarts through the kick, even though the exit handler's own BUG-050 roll fires as a side effect of the same exit.
- [x] 11.6 Sync the `hera-coordination` spec (both `openspec/specs/` and this archived snapshot) with a new "The size-drift kick refuses to auto-restart a closed-out worker" requirement, sibling to Stage 9's Enter requirement, plus this change's own `design.md`/`proposal.md` prose, and the `hera-view.md`/`orchestration.md` gotchas.
- [x] 11.7 Run `make pre-pr` and confirm it passes clean. Commit locally only — no push, no PR, per the coordinator's instruction; report back via `hera_send` when committed.
