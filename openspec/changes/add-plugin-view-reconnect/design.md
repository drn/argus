...
- **Warm-restart first-frame race (found in live dogfood 2026-06-11; fixed).** A plugin daemon that restarts warm renders its first frame within ~35ms of the new WebSocket connecting — before it applies argus's initial resize — then stays stuck at its default (tiny) size until the next resize it processes. argus logged the correct size on resume yet the pane rendered tiny and self-healed only on a manual terminal resize. Fix: `scheduleResumeResize` re-sends the resize envelope once more `a.resumeResizeDelay` (300ms) after a successful resume, forcing past the dedupe, so the plugin gets a resize after it has settled. Guarded against teardown / a second bounce within the window. A cold initial activation doesn't hit this (the daemon is ~188ms to first-frame, so the single initial resize lands first). If a future plugin dedupes identical-size resizes and this same-size re-send stops healing, switch to a transient size nudge.
...
- **Dead-connection-in-adoption-window (sub-millisecond, accepted).** If a freshly redialed connection dies in the tiny window after `Dial` returns nil but before `finishPluginReconnect`'s adoption closure runs, the new connection's `onClose` may be queued before adoption; its stale-drop guard (`m.conn == dropped`) then fails (m.conn is still the old connector), and after adoption the reconciler sends to a dead socket (logged once per draw streak) with no auto-restart. In practice tview's FIFO queue runs the adoption closure first (it's queued synchronously in the loop before the new pump can error), so the disconnect closure sees the adopted conn and restarts correctly — the failure only manifests if the new socket dies faster than the loop goroutine returns from `Dial`. The user recovers with Esc/Ctrl+Q. Not worth the extra `pendingConn` machinery for a single-user tool; documented rather than guarded.
