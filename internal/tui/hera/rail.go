...
// SubtreeNeedsInput a folded header always shows for any descendant: a
// descendant that actually needs input is its own, separate candidate, so
// counting the rollup too would double-count it. SelectByTaskID gained a
// matching header-row scan, so a coordinator's own need is now reachable
// rather than a "found but unreachable" dead cycle stop. A NESTED
// sub-coordinator that bridges as an ordinary role-bearing worker row in its
// PARENT orchestrator (appendWorkerRow) is unaffected either way — its own
// need was already a reachable candidate here. A non-selectable row (e.g. a
// pinned entry's non-selectable continuation line) never qualifies, mirroring
