...
// occupies (D9 + BUG-006): the node view's worst case — name / status / up to
// descMaxLines description lines / feeds — which also covers the 3-line group
// view (range·title / members / downstream, padded). Held constant (derived from
// descMaxLines) so the diagram budget never drifts with the cursor target.
const headerContentRows = 3 + descMaxLines // name + status + feeds (3 fixed) + description lines
...
func (w *Widget) nodeHeaderLines(id string) []string {
	...
	lines := make([]string, 0, 2+len(descLines)+1)
	lines = append(lines, n.Name, statusLine)
