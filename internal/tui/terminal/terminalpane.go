...
func (tp *TerminalPane) PasteHandler() func(pastedText string, setFocus func(p tview.Primitive)) {
	return tp.WrapPasteHandler(func(pastedText string, setFocus func(p tview.Primitive)) {
		...
		if sess != nil && sess.Alive() {
			...
			_, _ = sess.WriteInput([]byte(data), agentview.OriginUser)
		}
	})
}
...
// dropped.
//
// Session.WriteInput suppresses duplicate OSC 10/11 answers after its own
// startup color reply; other terminal-query answers still pass through.
...
// OriginUser would stamp lastUserInput exactly like a real keystroke, which
// idle-detection's clear-on-input logic reads to decide the user has
// responded — falsely clearing a still-unanswered needs-input flag every
// time the agent CLI's own redraw re-triggers a capability query, which
