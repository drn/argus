...
func TestPruneCompleted_WithWorktrees_ShowsModal(t *testing.T) {
	...
	// Simulate first parallel worktree done — still waiting for second
	...
	// Simulate second parallel worktree done — all done
	updated, _ = um.Update(PruneProgressMsg{})
	um = updated.(Model)
...
func TestAgentView_CtrlP_OpensPR(t *testing.T) {
	task := &model.Task{
	...
	// ctrl+p should open the PR regardless of session state.
...
func TestAgentView_CtrlP_NoPR(t *testing.T) {
	...
	// ctrl+p with no PRURL should return nil cmd.
	msg := tea.KeyMsg{Type: tea.KeyCtrlP}
	_, cmd := m.Update(msg)
	if cmd != nil {
		t.Error("expected nil cmd when task has no PRURL")
