	...
	},
	{
		...
		InputSchema: map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"model":        map[string]interface{}{"type": "string", "description": "(optional) Per-worker model override; choose by task complexity. Must be valid for the worker's resolved backend (claude: opus/sonnet/haiku/fable; codex: e.g. gpt-6-sol; pi: its model ids; opencode: a provider-qualified provider/model id, e.g. opencode/space-bunny-free). Empty = backend default. Delivered through the backend's supported mechanism (--model for claude/codex/pi, child-only OPENCODE_CONFIG_CONTENT for opencode); ignored if the backend command already hard-codes a model"},
