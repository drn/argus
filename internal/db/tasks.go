...
const taskColumns = `id, name, status, project, branch, prompt, backend, model, worktree, agent_pid, session_id, sandboxed, archived, pinned, base_branch, result, archetype, created_at, started_at, ended_at`
...
func scanTask(row scanner) (*model.Task, error) {
	...
	if err := row.Scan(&t.ID, &t.Name, &status, &t.Project, &t.Branch, &t.Prompt, &t.Backend, &t.Model, &t.Worktree, &t.AgentPID, &t.SessionID, &sandboxed, &archived, &pinned, &t.BaseBranch, &t.Result, &t.Archetype, &createdAt, &startedAt, &endedAt); err != nil {
		return nil, err
	}
...
func (d *DB) addLocked(t *model.Task) error {
	...
	_, err := d.conn.Exec(`INSERT INTO tasks (id, name, status, project, branch, prompt, backend, model, worktree, agent_pid, session_id, sandboxed, archived, pinned, base_branch, result, archetype, created_at, started_at, ended_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		t.BaseBranch, t.Result, t.Archetype,
...
func (d *DB) updateLocked(t *model.Task) (model.Status, bool, bool, error) {
	...
	res, err := d.conn.Exec(`UPDATE tasks SET name=?, status=?, project=?, branch=?, prompt=?, backend=?, model=?, worktree=?, agent_pid=?, session_id=?, sandboxed=?, archived=?, pinned=?, base_branch=?, result=?, archetype=?, created_at=?, started_at=?, ended_at=? WHERE id=?`,
