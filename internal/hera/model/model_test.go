...
// The tests below directly cover model.go helpers that are also exercised
// indirectly via internal/tui/hera's much larger test suite (Rail, Selection,
// plan view) — but that package-external exercise doesn't count toward THIS
// package's own coverage (Go coverage is per-instrumented-package unless
// -coverpkg is used). Added so internal/hera/model stays independently
// well-tested without relying on a sibling package's test run.

func TestModel_RoleByID(t *testing.T) {
	o := OrchView{ID: 1, Name: "R", Roles: []RoleView{
		{RoleID: 10, OrchID: 1, Name: "coord", Kind: db.HeraKindCoordinator},
		{RoleID: 11, OrchID: 1, Name: "w", Kind: db.HeraKindWorker},
	}}
	m := Model{Active: []OrchView{o}}

	got := m.RoleByID(11)
	testutil.NotNil(t, got)
	testutil.Equal(t, got.Name, "w")

	testutil.Nil(t, m.RoleByID(99999))
	// Freelance roles aren't searched (no owning orchestrator to canonicalize).
	free := Model{Freelance: []RoleView{{RoleID: 5, Name: "free"}}}
	testutil.Nil(t, free.RoleByID(5))
}

func TestModel_RoleOrchID(t *testing.T) {
	o := OrchView{ID: 1, Name: "R", Roles: []RoleView{
		{RoleID: 10, OrchID: 1, Name: "coord", Kind: db.HeraKindCoordinator},
		{RoleID: 11, OrchID: 1, Name: "w", Kind: db.HeraKindWorker},
	}}
	...
	orchID, ok := m.RoleOrchID(11)
	testutil.Equal(t, ok, true)
	testutil.Equal(t, orchID, int64(1))

	_, ok = m.RoleOrchID(99999)
	testutil.Equal(t, ok, false)
}

func TestModel_CanonicalParents_WorkerBridge(t *testing.T) {
	// R(coord tr, worker bridge→tc) → C(coord tc).
	r := orchView(1, "R", "tr", wk("bridge", "tc"))
	c := orchView(2, "C", "tc")
	m := Model{Active: []OrchView{r, c}}

	parents := m.CanonicalParents()
	cp, ok := parents[2]
	testutil.Equal(t, ok, true)
	testutil.Equal(t, cp.OrchID, int64(1))
	testutil.Equal(t, cp.CoordSpawn, false)

	// The root has no canonical parent.
	_, ok = parents[1]
	testutil.Equal(t, ok, false)
}

func TestModel_BridgeParentOf(t *testing.T) {
	t.Run("top-level has no parent", func(t *testing.T) {
		m := Model{Active: []OrchView{orchView(1, "R", "tr")}}
		_, _, ok := m.BridgeParentOf(1)
		testutil.Equal(t, ok, false)
	})

	t.Run("worker-bridge child resolves parent orch and bridging role", func(t *testing.T) {
		r := orchView(1, "R", "tr", wk("bridge", "tc"))
		c := orchView(2, "C", "tc")
		m := Model{Active: []OrchView{r, c}}
		bridgeRole := roleByName(t, &m, 1, "bridge")

		orchID, roleID, ok := m.BridgeParentOf(2)
		testutil.Equal(t, ok, true)
		testutil.Equal(t, orchID, int64(1))
		testutil.Equal(t, roleID, bridgeRole.RoleID)
	})

	t.Run("coordinator-spawned sub-team resolves parent's coordinator role", func(t *testing.T) {
		p := coordOf(1, "P", 100, "T")
		q := coordOf(2, "Q", 200, "T")
		m := Model{Active: []OrchView{p, q}}
		...
		testutil.Equal(t, ok, true)
		testutil.Equal(t, orchID, int64(1))
		testutil.Equal(t, roleID, int64(100)) // P's coordinator role id
	})

	t.Run("unknown child orchestrator", func(t *testing.T) {
		m := Model{}
		_, _, ok := m.BridgeParentOf(999)
		testutil.Equal(t, ok, false)
	})
}

func TestOrchView_BridgingRoleFor(t *testing.T) {
	o := orchView(1, "R", "tr", wk("bridge", "tc"))
	got := o.BridgingRoleFor("tc")
	testutil.NotNil(t, got)
	testutil.Equal(t, got.Name, "bridge")

	testutil.Nil(t, o.BridgingRoleFor("no-such-task"))
	// The coordinator role itself is never a bridging row.
	testutil.Nil(t, o.BridgingRoleFor("tr"))
}

func TestOrchView_HasWorkerBridging(t *testing.T) {
	o := orchView(1, "R", "tr", wk("bridge", "tc"))
	testutil.Equal(t, o.hasWorkerBridging("tc"), true)
	testutil.Equal(t, o.hasWorkerBridging("nope"), false)
}

func TestFormatCostUSD(t *testing.T) {
	testutil.Equal(t, FormatCostUSD(0), "$0.00")
	testutil.Equal(t, FormatCostUSD(0.005), "<$0.01")
	testutil.Equal(t, FormatCostUSD(1.5), "$1.50")
}

func TestModel_AnnotateRoles(t *testing.T) {
	r := orchView(1, "R", "tr", wk("w", "tw"))
	m := Model{
		Pinned:    []OrchView{orchView(2, "P", "tp")},
		Active:    []OrchView{r},
		Archived:  []OrchView{orchView(3, "A", "ta")},
		Freelance: []RoleView{{RoleID: 9, Name: "free"}},
	}
	var touched []string
	m.AnnotateRoles(func(rv *RoleView) {
		touched = append(touched, rv.Name)
		rv.Archetype = "stamped"
	})
	testutil.Equal(t, len(touched), 5) // coord+worker in Active, coord in Pinned, coord in Archived, plus freelance
	testutil.Equal(t, roleByName(t, &m, 1, "w").Archetype, "stamped")
	testutil.Equal(t, m.Freelance[0].Archetype, "stamped")

	// nil fn is a no-op, not a panic.
	m.AnnotateRoles(nil)
}

func TestModel_UnmanagedNeedsInputCount(t *testing.T) {
	o := orchView(1, "R", "tr", wk("w", "tw"))
	m := Model{Active: []OrchView{o}}

	testutil.Equal(t, m.UnmanagedNeedsInputCount(nil), 0)

	needsInput := map[string]bool{"tw": true, "unmanaged-task": true}
	testutil.Equal(t, m.UnmanagedNeedsInputCount(needsInput), 1) // "tw" is managed (bound), the other isn't
}

func TestModel_NeedsInputTotalCount_And_NeedsInputRoleIDs(t *testing.T) {
	o := orchView(1, "R", "tr", wk("w", "tw"))
	m := Model{Active: []OrchView{o}}
	testutil.Equal(t, m.NeedsInputTotalCount(), 0)
	testutil.Nil(t, m.NeedsInputRoleIDs())

	roleByName(t, &m, 1, "w").NeedsInput = true
	testutil.Equal(t, m.NeedsInputTotalCount(), 1)
	ids := m.NeedsInputRoleIDs()
	testutil.Equal(t, len(ids), 1)
	testutil.Equal(t, ids[roleByName(t, &m, 1, "w").RoleID], true)
}
...
func TestModel_OrchIDsForTask(t *testing.T) {
	...
	ids := m.OrchIDsForTask("shared")
	testutil.Equal(t, len(ids), 2)
	testutil.Nil(t, m.OrchIDsForTask("missing"))
}

func TestSelection_StatusRole(t *testing.T) {
	coordOrch := &OrchView{Roles: []RoleView{{Kind: db.HeraKindCoordinator, Name: "coord", Live: true, TaskID: "tc"}}}
	t.Run("role selected wins", func(t *testing.T) {
		s := Selection{Role: &RoleView{Name: "w"}, Orch: coordOrch}
		testutil.Equal(t, s.StatusRole().Name, "w")
	})
	t.Run("header selection resolves the orchestrator's coordinator", func(t *testing.T) {
		s := Selection{Role: nil, Orch: coordOrch}
		testutil.Equal(t, s.StatusRole().Name, "coord")
	})
...
func TestSelection_IsWorkerOrFreelance(t *testing.T) {
	testutil.Equal(t, Selection{Role: &RoleView{Kind: db.HeraKindWorker}}.IsWorkerOrFreelance(), true)
	testutil.Equal(t, Selection{Role: &RoleView{Kind: db.HeraKindFreelance}}.IsWorkerOrFreelance(), true)
	testutil.Equal(t, Selection{Role: &RoleView{Kind: db.HeraKindCoordinator}}.IsWorkerOrFreelance(), false)
	testutil.Equal(t, Selection{}.IsWorkerOrFreelance(), false)
}

func TestSelection_IsCoordinator(t *testing.T) {
	testutil.Equal(t, Selection{Role: &RoleView{Kind: db.HeraKindCoordinator}}.IsCoordinator(), true)
	testutil.Equal(t, Selection{Role: &RoleView{Kind: db.HeraKindWorker}}.IsCoordinator(), false)
	testutil.Equal(t, Selection{Orch: &OrchView{}}.IsCoordinator(), true) // header selection
	testutil.Equal(t, Selection{}.IsCoordinator(), false)
}

func TestSelection_CoordTaskID(t *testing.T) {
	testutil.Equal(t, Selection{}.CoordTaskID(), "")
	o := &OrchView{Roles: []RoleView{{Kind: db.HeraKindCoordinator, Live: true, TaskID: "tc"}}}
	testutil.Equal(t, Selection{Orch: o}.CoordTaskID(), "tc")
}

func TestRoleView_NeedsInputOwn_SustainedActiveSuppresses(t *testing.T) {
	r := &RoleView{NeedsInput: true, SustainedActive: true}
	testutil.Equal(t, r.needsInputOwn(), false)
}

func TestOrchView_CoordRole_None(t *testing.T) {
	o := &OrchView{Roles: []RoleView{{Kind: db.HeraKindWorker}}}
	testutil.Nil(t, o.CoordRole())
}

func TestModel_BridgeSubtree_UnknownRoot(t *testing.T) {
	m := Model{}
...
func TestSelection_KanbanTarget(t *testing.T) {
	orch := &OrchView{ID: 1}
	t.Run("top-level orchestrator header qualifies", func(t *testing.T) {
		s := Selection{Orch: orch, TopLevelOrch: true}
		testutil.Equal(t, s.KanbanTarget(), orch)
	})
	t.Run("a role selection never qualifies", func(t *testing.T) {
		s := Selection{Role: &RoleView{}, Orch: orch, TopLevelOrch: true}
		testutil.Nil(t, s.KanbanTarget())
	})
	t.Run("a nested (non-top-level) orchestrator header never qualifies", func(t *testing.T) {
		s := Selection{Orch: orch, TopLevelOrch: false}
		testutil.Nil(t, s.KanbanTarget())
	})
	t.Run("empty selection never qualifies", func(t *testing.T) {
