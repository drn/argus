...
func CreateAndStart(database *db.DB, runner SessionProvider, input CreateInput) (*model.Task, SessionHandle, error) {
	...
	// captured post-exit.
	if resolved, berr := ResolveBackend(task, cfg); berr == nil && !IsCodexBackend(resolved.Command) && !IsPiBackend(resolved.Command) && !IsOpencodeBackend(resolved.Command) {
	...
	// that stack — a later runner.Start failure unwinds the binding too.
