...
- **Use `msg.Type` not `msg.String()` for Bubble Tea key matching.** String comparison fails when terminals encode modifier keys differently (e.g., urxvt sends `\x1b[Od` for ctrl+left, which Bubble Tea parses as `KeyCtrlLeft` with `Alt=true` → `String()` returns `"alt+ctrl+left"` instead of `"ctrl+left"`). Always match on `msg.Type` and check `msg.Alt` separately. For agent view pane switching, ctrl+left/right arrows are the only binding — plain left/right arrows are passed through to the agent process.
...
- **`removeWorktree` must validate paths before `os.RemoveAll`.** The `os.RemoveAll` fallback in `removeWorktree` will nuke any directory if `git worktree remove` fails — including the root project if `t.Worktree` is incorrectly set. The `isWorktreeSubdir` guard ensures the path contains `/.claude/worktrees/` before any removal. Any new cleanup code that calls `os.RemoveAll` on user-provided paths must have a similar safety check.
- **`textarea.LineCount()` only counts hard newlines, not soft wraps.** A long line that visually wraps to 3 lines still reports `LineCount() == 1`. For auto-resize modals, compute visual lines by dividing each hard line's rune length by the textarea width and rounding up. Without this, the textarea scrolls internally instead of expanding.
