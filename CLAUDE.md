...
- **`screen.Sync()`** is for repairing screen damage caused by **something outside our control** writing to the same terminal — exactly two callsites: Ctrl+L (user-initiated refresh) and tmux focus regain (window switch may have repainted our pane from a stale backing). Both are rare; one `CSI 2J` flash per occurrence is the correct tradeoff. Anywhere else, **don't call Sync.**
