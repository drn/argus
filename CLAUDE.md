...
- **A new skill under `.claude/skills/` intended for use outside this repo (i.e. not gated purely to `ARGUS_TASK_ID`/argus-specific behavior) must be promoted via `/promote` at or before its merge reaches Aaron's canonical checkout.** `~/.claude/skills/<name>` entries are manually-created symlinks into the canonical checkout at `/Users/aaron/Development/Personal/argus/.claude/skills/<name>` — nothing auto-promotes a new skill, so until someone runs `/promote` after the code lands there, the skill is invisible to any Claude session running outside this repo, however useful its convention. This applies even when the skill lands on a long-lived non-`master` integration branch (e.g. `argus/model-tiering`) — promotion still can't happen until that branch reaches the canonical checkout. (PR #845 proposes a different, narrower fix for a curated whitelist of argus-coupled skills — embed + `--add-dir`, no manual step at all — but as of this writing covers only `archive`/`argus-complete`/`argus-schedule`/`hera`/`hera-plan`; it does not include the profile/review skills, including this one. If #845 lands, extending its embed list to cover those is the natural way to retire this bullet for the skills it covers.)
