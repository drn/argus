...
- `internal/tui/hera/` — **Native Hera view** (the second tab when `hera.enabled`). Three-region layout: a left **rail** (orchestrators with their coordinator/worker roles, plus Pinned/Freelance/Archive sections), a middle **coordinator pane** (the selected orchestrator's coordinator PTY), and a right **details region** that shows a worker's live terminal, or — for a coordinator — a read-only roster that toggles (`g`) to the embedded dependency DAG. Panes are fed by polling the in-process runner ring (no SSE/proxy fan-out). `model.go`/`rail.go`/`panes.go`/`details.go`/`focus.go`/`ops.go`/`refresher.go` split state, rendering, the focus ladder, mutations (over the M1 store), and a goroutine-free debounced refresher.
...
**Hera (native multi-agent coordination):** Hera roles (coordinator / worker / freelance) are bound to argus tasks and addressed by name; everything else reuses primitives Argus already owns. The message bus is the existing `notify.Notifier` (idle-gated, exactly-once), born-bound worker spawn reuses `agent.CreateAndStart` with a transactional role-binding insert, and the dependency DAG is the native `task_link`/`deps` graph folded into the Hera view's details pane. The store enforces one live binding per (task, orchestrator) but allows a task to participate in several orchestrators. Coordination runs in-process in the daemon; the native Hera view renders directly against the host `tcell.Screen`. Gated by `cfg.Hera.Enabled` (default ON); set it `false` to relabel the second tab back to the legacy DAG-only view and fall back to the external Hera *plugin* (which keeps its own `~/.hera` state and runs over the plugin substrate). Native Hera and the plugin are mutually exclusive per the flag and share no state — native starts fresh, with no migration of prior `~/.hera` data.
