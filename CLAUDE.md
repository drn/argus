...
- **Never call slow external probes (npx, network) unconditionally at startup.** `IsSandboxAvailable()` uses `exec.Command("npx", ...)` which can block for seconds. Gate expensive detection behind the feature's enable flag (e.g., `cfg.Sandbox.Enabled && IsSandboxAvailable()`) and always use `context.WithTimeout`. Use `--no` not `--yes` with `npx` to avoid silently downloading packages — detection should be read-only.
