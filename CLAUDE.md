...
- **srt path format: `//` means "absolute from root", so strip the leading `/`.** The `@anthropic-ai/sandbox-runtime` uses `//` prefix for absolute paths (e.g., `//home/user/wt` encodes `/home/user/wt`). Naively concatenating `"//" + absolutePath` produces `"///home/..."` (triple slash) which may not be recognized. Always strip the leading `/` first: `"//" + strings.TrimPrefix(path, "/")`. The `normalizeSrtPath` helper in `internal/agent/sandbox.go` handles this.
- **`IsSandboxAvailable()` is cached via `sync.Once` — safe to call unconditionally.** The first call may be slow (npx probe with 5s timeout), but all subsequent calls return the cached result instantly. Always use `--no` not `--yes` with `npx` to avoid silently downloading packages — detection should be read-only. The settings view calls `IsSandboxAvailable()` on every refresh to show accurate install status regardless of whether sandbox is enabled.
