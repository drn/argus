...
**UX-tearing prevention — the branch-change AND content-change contracts:** Two complementary callbacks. (1) **`OnBranchChange`** — fires when a widget's `Draw` conditionally renders a different rendering branch in the same rect (mode swap, conditional row, scrollback toggle, diff view, banner overlay, filter input, focus-driven panel swap). App wires to `forceRedraw` → sets `pendingSync` → `afterDraw` Syncs unconditionally. tcell's per-cell diff is correct only when the SET of cells written each frame is the same as last frame; branch swaps shift the cell set and leave ghosts (duplicated keybars, bleed-through project headers, status bars at two y-positions). Structural hooks: `pages.SetChangedFunc`, `tasklist.OnLayoutChange`/`OnFilterToggle`, `filePanel.OnLayoutChange`, `agentPane.OnBranchChange`, `taskPreview/taskDetail/taskGitPanel/gitPanel/dagWidget.OnBranchChange`, `settings.OnBranchChange`. (2) **`OnContentChange`** — fires when a widget streams new cell content within the SAME rendering branch (preview pane RefreshOutput grid rebuild, terminal pane PTY byte consumption). App wires to `forceContentSync` → sets `pendingContentSync` → `afterDraw` Syncs only when `multiplexerMode` is true. Bare terminals trust tcell.Show()'s diff for these updates (free, no flash). Inside tmux, Show()'s SGR/cursor optimization can desync from the multiplexer's pane backing, so the content-update path needs a Sync to recover. Structural hooks: `taskPreview.OnContentChange`, `agentPane.OnContentChange`. **When you add a new conditional render branch, wire `OnBranchChange` AND add a smoke test (pattern: `TestSmoke_FilterToggleFiresRedraw`). When you add a widget that streams cells within a stable branch, wire `OnContentChange` AND add a smoke test (pattern: `TestSmoke_TaskPreviewContentSyncInMultiplexer`).** The standard widget shape: `OnBranchChange func()` + `notifyBranchChange()`, and/or `OnContentChange func()` + `notifyContentChange()`. See `gotchas/ui-threading.md`.
