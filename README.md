...
| `POST` | `/api/tasks/{id}/resize` | Register a viewer's size: `{"cols":N,"rows":M,"conn":"<id>"}`. The PTY is sized to the per-dimension **min over all active viewers** (TUI pane, other web tabs, `--remote` TUIs) — smallest wins, tmux-style — not set absolutely. `conn` ties this claim to the same client's output stream (drop on disconnect); omit it for a legacy direct resize. Returns `{cols,rows,rerendered}` where `cols`/`rows` are the **effective** (min-over-viewers) size the PTY applied, which may be smaller than requested when another viewer pins it; `rerendered:true` means the effective width crossed the rerender margin (≥15 col delta from session-start width) and the daemon queued a kill+resume so the agent re-emits scrollback at the new width. The SPA's exit-event handler reattaches automatically. |
| `POST` | `/api/tasks/{id}/viewer/release` | Drop a viewer's size claim without closing its stream: `?conn=<id>` (query param so `sendBeacon` can call it on tab-hide/unload). The PTY grows back to the min over the remaining viewers. 400 if `conn` is missing, 404 if no live session. |
