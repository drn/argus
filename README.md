...
| `Enter`              | On a sub-coordinator node, drill into its child orchestrator's plan; on a plain leaf, jump to that node's role within the Hera view (selects it in the rail + focuses its agent pane — no tab switch) |
