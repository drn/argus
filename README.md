...
| Method | Endpoint    | Description                                                                                                                                                                                                                            |
| ------ | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/api/hera` | Read-only orchestration roster. Returns `{orchestrators:[{id,name,pinned,archived,subtree_cost_usd,roles:[…]}], freelance:[…]}`. Each role carries `kind` (coordinator/worker/freelance), `status`, bound `task_id`/`task_name`/`task_status`, `live`, `ready_to_close`, per-rate-class token totals (`tokens_input`/`tokens_cache_write_1h`/`tokens_cache_write_5m`/`tokens_cache_read`/`tokens_output`), and `cost_usd` — all omitted (not `0`) when never measured. `subtree_cost_usd` sums the orchestrator's own roles only, not a recursive walk into nested sub-coordinators (see `internal/pricing`). Feeds the webapp's **Hera** tab. |
