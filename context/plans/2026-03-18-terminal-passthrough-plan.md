...
- [x] Implement the top-level layout shell in `internal/tui`: `Header` (tab bar), `StatusBar` (bottom hints + counts), `TaskListView` (grouped by project with archive), `AgentPane` (placeholder), `SidePanel` (git/files)
...
- [x] Add `internal/tui/terminalpane.go` — custom `tview.Box`-based widget with native PTY rendering via vt10x cells → tcell.Screen cells, cursor display, input row highlighting, and scrollback
...
- [x] Exported `ReplayVT10X` and `EstimateVTRows` from `internal/ui/vtrender.go` for use by tui scrollback; kept `findInputRow` and cursor synthesis for Bubble Tea fallback
...
- [x] Ported git status panel (`internal/tui/gitstatus.go` — `GitPanel` with status/diff/branch sections)
- [x] Ported file explorer (`internal/tui/fileexplorer.go` — `FilePanel` with auto-expand, cursor nav, status icons)
- [x] Ported new task form (`internal/tui/newtaskform.go` — modal with project/backend selectors, prompt input)
...
- [x] Added regression tests: 40+ tests across all tui components (terminalpane, newtaskform, fileexplorer, gitstatus, app, tasklist)
...
- [x] Replace `vt10x.New(vt10x.WithSize(w, h))` → `vt.NewSafeEmulator(w, h)` in `internal/tui/terminalpane.go`
...
- [x] Remove tui's dependency on `internal/ui/vtrender.go` exports (`ReplayVT10X`, `EstimateVTRows`)

**Tests:**
- [x] Update `internal/tui/terminalpane_test.go` for x/vt API
...
**Update imports:**
- [x] `internal/tui/app.go`: `ui.FetchGitStatus` → `gitutil.FetchGitStatus`, `ui.ParseGitStatus` → `gitutil.ParseGitStatus`, etc.
- [x] `internal/tui/fileexplorer.go`: `ui.ChangedFile` → `gitutil.ChangedFile`
- [x] `internal/tui/newtaskform.go`: skill loading → `skills.ScanSkills`
- [x] Keep `internal/ui/` files importing from `gitutil`/`skills` so BT runtime still compiles (deleted in Phase 11)

**vtrender.go stays in `internal/ui/`** — after Phase 6 removes tui's dependency on it, vtrender.go is only used by the BT runtime and will be deleted in Phase 11. No extraction needed.

### Phase 8: Port Reviews tab to tcell
**Status:** complete

New file: `internal/tui/reviews.go`
...
New file: `internal/tui/settings.go`
...
- [x] `internal/tui/projectform.go` — add/edit project (name, path, branch, backend selector, per-project sandbox settings: enabled inherit/true/false, deny-read paths, extra-write paths). Auto-detect remote default branch on path entry (prefer `upstream` over `origin`).
- [x] `internal/tui/backendform.go` — add/edit backend (name, command, prompt flag). Edit mode: name field read-only. Mirrors `internal/ui/backendform.go` pattern with 3 text input fields.
- [x] `internal/tui/renametask.go` — rename task (single text input). Display-only rename: updates `t.Name` in DB, worktree dir/branch/session unchanged. Works while agent is running.
- [x] `internal/tui/kbvaultform.go` — KB vault path editor. Takes DB config key explicitly (`"kb.metis_vault_path"`, `"kb.argus_vault_path"`), not derived from label string.
...
- [x] Verify: `grep -r "internal/ui" internal/tui/ cmd/` → zero matches (only `internal/gitutil` and `internal/skills`)
...
- After Phase 11: verify no `internal/ui` imports remain in tui or cmd.
