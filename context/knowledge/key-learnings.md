...
- **Tab/Backtab must accept open AC before advancing focus.** Without calling `projACAccept()`/`branchACAccept()` first, Tab closes the dropdown but leaves partial text in the field — the user gets an invalid project/branch name that doesn't match any option.
