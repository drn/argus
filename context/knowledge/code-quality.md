...
**Flow**: `handleAgentKey` → `Ctrl+/` (dual-encoding: `KeyCtrlUnderscore` OR `KeyRune '/' + ModCtrl`) → `openAgentLinks` → background goroutine reads `agent.SessionLogPath(taskID)` → `ExtractLinks` → `QueueUpdateDraw` with mode guard → `openFuzzyLinkPickerModal` → modal handles keys → Enter → `openURL` → `closeFuzzyLinkPickerModal` → restore `modeAgent`.
