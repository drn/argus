...
## add-mac-hera-rail-toggle: sidebar-mode Hera tree — pure decision logic ahead of SwiftUI, single poll loop

**Both the sidebar Hera-tree grouping (`ArgusKit/HeraTree+Builder.swift`, `HeraTreeBuilder.build(_:HeraRoster) -> HeraTree`) and the dual-pane detail-pane selection (`ArgusKit/HeraDetailPaneResolver.swift`, resolving `(roster, selectedRoleID) -> HeraDetailPanes`) are pure, dependency-free functions living in ArgusKit, not the `Argus` app target** — mirroring the existing `TaskGrouping.swift`/`FileTree+Builder.swift` precedent. This is how they're unit-tested (`HeraTreeBuilderTests.swift`, `HeraDetailPaneResolverTests.swift`) despite this codebase having no SwiftUI/AppState test harness: every actual decision (which orchestrators nest under which bridging role, which pane is a terminal vs a read-only roster vs empty) lives in a plain function, leaving only thin, untested rendering glue in the `Argus`-target views. Reach for the same split before writing new Hera-related decision logic directly into a `View`.

**`HeraDetailView` runs NO roster-polling loop of its own — it reads the shared `AppState.currentHeraRoster` cache that `HeraTreeSidebar` already polls on its own ~5s cadence.** The two are always mounted together (sidebar/detail slots of the same `NavigationSplitView` whenever `sidebarMode == .hera`), so a second independent poll would only double the `/api/hera` request rate with no freshness gain. If `HeraDetailView` is ever made to render standalone without its sidebar sibling, it needs its own poll wired first — don't assume the cache populates itself.
