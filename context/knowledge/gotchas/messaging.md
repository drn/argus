...
- **The `i` inbox viewer must never ack or mark read** — it reads `task_messages` via `Inbox(UnreadOnly=false)` and hera messages via `HeraMessagesToRoles` (all read states), never `AckMessages`/`MarkHeraMessagesRead`. Acking from a debugging view would change what the agent sees on its next `task_inbox`/`hera_inbox` and corrupt the delivery behaviour being inspected. Guarded by `TestLoadInboxEntries` + `TestSmoke_InboxModal` unread-count assertions.
- **`HeraInbox` is unread-only, so it is useless for a history view** — hera messages for a task are found by walking `ListHeraBindingsByTask` (ended bindings included, so a revived/moved task still shows old mail) and feeding every role ID to `HeraMessagesToRoles`, which keeps the newest N and returns them oldest-first.
- **Inbox modal titles must stay ASCII** — `DrawBorderedPanel` lays titles out byte-wise, so an em dash renders as `—──` garbage. Body text is fine (`DrawText` is rune-aware).
