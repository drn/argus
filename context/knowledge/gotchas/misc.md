...
- **`bareLinkRe` must exclude `"`, backtick, `{`, `}`, `<`.** These characters are never valid in URLs per RFC 3986. Without exclusion, the regex matches through quoted URL data (e.g., `"url",field,"url"`) producing garbage entries in the link picker.
