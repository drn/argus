...
- **`ActivateCursor` (Enter/Space) on a fanned-group MEMBER navigates, it does NOT collapse (BUG-013).** With a group fanned and the cursor on an interior member (`cursor.Member >= 0`), Enter/Space fires the member's leaf action via `CurrentNodeID()` (`OnEnter`, or `OnDrillIn` when Drillable) — exactly like a lone leaf. Only a COLLAPSED group fans out, and only a fanned ENCLOSURE with no member under the cursor (`Member < 0`) toggles collapse. Collapsing a fanned group is `Esc`'s job (`EscBack`), never Enter/Space-on-member. Because fanning lands the cursor on member 0, Enter/Space never re-collapses a group in normal nav.
