...
- **Three TUI sites still type-assert `a.db.(*db.DB)` and gracefully degrade in remote mode**: fork, schedule fire, prune-completed. They surface a status-bar error pointing to the REST equivalent. These ops only work locally because they shell out to git/PTY directly (fork additionally needs `OnWorktreeCreated` callbacks that run in the daemon's process); in remote mode the user must hit the REST endpoint from the daemon's host.
